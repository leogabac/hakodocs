...
Python has a built-in [`venv`](https://docs.python.org/3/library/venv.html){:.doc-link} module for creating virtual environments. It is simple and sometimes limited, but if you do not have access to fancier tools on a shared server, it is still useful to know it exists.

```bash
python -m venv <path/to/virtual/environment>
```

For example, on one of the High Performance Computing (HPC) servers I use, there are multiple Python versions callable by `python3.12` or `python3.8`, but we do not have access to `virtualenv`. In that case, I manage environments either with `conda` or `venv`. To do this, call the Python binary you want followed by the module flag `-m venv`
...
- [Python: Creating Modules](/python/creating-modules/)
