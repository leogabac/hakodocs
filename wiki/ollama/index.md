...
This will download the model and start an interactive terminal prompt with the LLM.
...
Ollama has [limited support](https://ollama.com/blog/amd-preview) for AMD discrete GPUs because ROCm only supports a subset of newer cards officially. Nevertheless, it is still possible in some cases to offload Ollama workloads to unsupported integrated or discrete GPUs by changing the `HSA_OVERRIDE_GFX_VERSION` environment variable when launching Ollama.
